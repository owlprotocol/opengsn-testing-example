...
import { contracts, gsnContracts } from './artifacts';
...
async function configureGSN(web3HttpUrl: string, defaultAccount: string) {
    //Don't deploy duplicate paymaster
    const gsnEnv = await GsnTestEnvironment.startGsn(web3HttpUrl);
    //Wait on port
    //@ts-ignore
    console.debug(`OpenGSN running at ${gsnEnv.relayUrl}`);

    const gsnProvider = gsnEnv.relayProvider;
    //let web3GSN = new Web3(gsnProvider)
    //web3GSN.eth.defaultAccount = defaultAccount
    for (const k in gsnContracts) {
        //@ts-ignore
        gsnContracts[k].setProvider(gsnProvider);
        //@ts-ignore
        gsnContracts[k].web3.eth.defaultAccount = defaultAccount;
    }
...
let gsnEnv: TestEnvironment | undefined;
...
export async function configure(config?: FullTestConfig) {
    ...
    if (!isEqual(lastConfig, config) || !initialConfig) {
        ...
        if (config?.gsn) {
            gsnEnv = await configureGSN(ganacheEnv.web3HttpUrl as string, accounts[0]);
        } else {
            gsnEnv = undefined;
        }
    }

    return { ganache: ganacheEnv, accounts, gsn: gsnEnv };
}
